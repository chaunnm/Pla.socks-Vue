...
export default {
  ...
  computed: {
    ...
    levelErrors() {
      ...
      !this.$v.level.required && errors.push("A category must have a level");
      return errors;
    },
    ...mapGetters({
      categoryItemById: "GET_CATEGORY_ITEM_BY_ID",
    }),
  },
  created() {
    this.name = this.categoryItemById(parseInt(this.cateId)).name;
    this.description = this.categoryItemById(parseInt(this.cateId)).description;
    this.level = this.categoryItemById(parseInt(this.cateId)).level;
    this.isChild = this.categoryItemById(parseInt(this.cateId)).isChild;
  ...
  methods: {
    submit() {
      ...
      if (this.$v.$invalid) {
        this.$toast.open({
          message:
            "There is some problems while updating category, please try again! ☹️",
          type: "error",
          duration: 2000,
          dismissible: true,
          position: "bottom",
        ...
        const updatedCategory = {
          ...
          createdAt: this.categoryItemById(parseInt(this.cateId)).createdAt,
          updatedAt: temp,
        };
        this.$store.commit("CHANGE_CATEGORY_ITEM", updatedCategory);
        this.$toast.open({
