    ...
    <div class="adminOrder-container">
        ...
        <div class="table">
            <v-card>
                ...
                <v-data-table dense :search="search" v-model="selected" :headers="headers" :items="orderList"
                    :single-select="singleSelect" item-key="orderId" show-select class="elevation-1" sort-by="calories">
                    <template v-slot:[`top`]> </template>
                    <template v-slot:[`item.orderId`]="{ item }">
                        <span style="color:blue;text-align:center;"> {{ item.orderId + 1 }} </span>
                    </template>
                    <template v-slot:[`item.orderInfo`]="{ item }">
                        <span>{{ item.orderInfo.fullname }}</span>
                    </template>
                    <template v-slot:[`item.status`]="{ item }">
                        <span :class="[Processing ? 'Processing' : '', Delivered ? 'Delivered' : '']">{{ item.status
                        }}</span>
                    </template>
