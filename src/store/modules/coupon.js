const coupon = {
  ...
  mutations: {
    ...
    CHANGE_COUPON_ITEM: (state, data) => {
        state.couponData.find(couponItem => couponItem.couponCode === data.couponCode).couponTitle = data.couponTitle;
        state.couponData.find(couponItem => couponItem.couponCode === data.couponCode).sale = data.sale;
        state.couponData.find(couponItem => couponItem.couponCode === data.couponCode).ship = data.ship;
        state.couponData.find(couponItem => couponItem.couponCode === data.couponCode).expiredAt = data.expiredAt;
        state.couponData.find(couponItem => couponItem.couponCode === data.couponCode).quantity = data.quantity;
      },
